...
@Service
public class NifiHostPublicDnsEntryService extends BasePublicEndpointManagementService {
    private static final Logger LOGGER = LoggerFactory.getLogger(NifiHostPublicDnsEntryService.class);
    ...
    private Map<String, String> doActionOnStack(
        ...
        if (stack.getCluster() != null && manageCertificateAndDnsInPem()) {
            LOGGER.info("Modifying DNS entries for NiFi nodes on stack '{}'", stack.getName());
            Cluster cluster = stack.getCluster();
            Map<String, String> ipsByFqdn = getBrokerIpsByFqdn(stack, cluster);
            if (!CollectionUtils.isEmpty(candidateAddressesByFqdn)) {
                LOGGER.info("Modifying DNS entries for NiFi nodes on stack '{}', whitelist of candidates for the update: '{}'", stack.getName(),
    ...
    private Map<String, String> getBrokerIpsByFqdn(Stack stack, Cluster cluster) {
        Map<String, List<String>> componentLocation = componentLocatorService.getComponentLocation(cluster, nifiConfigProvider.getRoleTypes());
    ...
    private Map<String, String> createOrUpdateDnsEntries(Map<String, String> ipsByFqdn, String environmentCrn) {
        LOGGER.info("Register DNS entries for NiFi nodes for FQDNs: '{}'", String.join(",", ipsByFqdn.keySet()));
    ...
    private Map<String, String> doDeregister(Map<String, String> ipsByFqdn, String environmentCrn) {
        LOGGER.info("Deregister DNS entries for NiFi nodes for FQDNs: '{}'", String.join(",", ipsByFqdn.keySet()));
        String userCrn = ThreadBasedUserCrnProvider.getUserCrn();
        String accountId = ThreadBasedUserCrnProvider.getAccountId();
        DetailedEnvironmentResponse environment = environmentClientService.getByCrn(environmentCrn);
